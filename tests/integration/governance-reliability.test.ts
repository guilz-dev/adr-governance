...
describe('ADR scope and metadata', () => {
  it('does not collide with unrelated RFCs and recognizes five-digit ADR numbers', async () => {
    ...
    const evidence = await runAttest({ repoRoot: f.repo, config, baseRef: f.baseRef, adrIds: ['ADR-10000'], reviewedProposalIds: ['ADR-00001'] })
    expect((await runCheck(f.repo, { baseRef: f.baseRef, githubEventPath: await event(f.repo, toGitHubMarkdown(evidence)) })).issues).toEqual([])
